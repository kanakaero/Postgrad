...
class NN_mFF:
    def __init__(self, layers, bcs_samplers, res_samplers, u,a, b, sigma):
        ...
        self.W1 = tf.Variable(tf.random.normal([1, layers[0] //2], dtype=tf.float32) * 1, dtype=tf.float32, trainable=False)
        self.W2 = tf.Variable(tf.random.normal([1, layers[0] //2], dtype=tf.float32) * sigma, dtype=tf.float32, trainable=False)
    ...
    def initialize_NN(self, layers):
        ...
        b = tf.Variable(tf.random.normal([1, layers[-1]], dtype=tf.float32), dtype=tf.float32)
        weights.append(W)
        biases.append(b)
