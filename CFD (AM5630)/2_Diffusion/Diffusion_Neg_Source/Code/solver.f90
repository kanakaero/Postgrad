...
subroutine solver()
    ...
    do iter = 1, max_iter
        T(1,:,:) = T(2,:,:)  ! Store the previous iteration values
        print *, 'Solver Iteration: ', iter
        R(iter) = 0.0_dp   ! Initialize the residual array
        do i = 2, imax-1
            do j = 2, jmax-1
                ! update a_p to incorporate the unknown part of the source term
                a_p(i,j) = a_p(i,j) - (s_p(i,j)*dx(i)*dy(j)/T(1,i,j))
                ! Interior nodes
                T(2,i,j) = (a_e(i,j)*T(2,i+1,j) + a_w(i,j)*T(2,i-1,j) + a_n(i,j)*T(2,i,j+1) + a_s(i,j)*T(2,i,j-1) + s_u(i,j)) / a_p(i,j)
            end do
        end do
        ...
        do i = 2, imax-1
            do j = 2, jmax-1
                R(iter) = max(R(iter), abs(T(2,i,j) - T(1,i,j)) / abs(T(2,i,j)))
            end do
        end do
    ...
    do i = 2, imax-1
        do j = 2, jmax-1
            dTdx = (T(2,i+1,j) - T(2,i-1,j)) / (2.0 * dx(i))
            dTdy = (T(2,i,j+1) - T(2,i,j-1)) / (2.0 * dy(j))
    ...
    do j = 1, jmax
        do i = 1, imax
            ...
            xc(i), yc(j), T(2,i,j), qx(i,j), qy(i,j), qmag(i,j)
        end do
    end do
